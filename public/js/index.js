$(document).ready(function () {
  ...
  $("#chatForm").submit(function () {
    $("#msgSending").html(
      "<img src='images/msgloader.GIF' style='float:left'>Message sending please wait..."
    );
    elem.scrollTop = elem.scrollHeight;

    var msg = $("#chatInput").val();
    socket.emit("chatMsg", {
      name: nm + " [" + window.datetime + "] ",
      msg: msg,
    });
    $("#chatInput").val("");
    return false;
  ...
  socket.on("msgEveryOne", function (data, messages) {
    $("#messages").append(
      '<p class="bubble-content"><b style="color:green"><i>' +
        data.name +
        ":</b></i> " +
        data.msg +
        "</p>"
  ...
  $("#startChat").click(function () {
    ...
    if (name == "") {
      ...
      $("#alertMsg").html(
        "<h3> space and special characters are not allowed!</h3>"
      );
      $("#myModal").modal("show");
    } else {
      ...
      socket.on("chkUser", function (chk) {
        if (chk > -1) {
          ...
          $("#alertMsg").html(
            "<h3> Nickname already taken choose other one!</h3>"
          );
          $("#myModal").modal("show");
        } else {
  ...
  socket.on("totalOnlineUser", function (data, me) {
    ...
    $.each(data, function (index, val) {
      $("#userList").append(
        '<li class="list-group-item " id="li_' +
          val +
          '"> <a href="javascript:void(0);" class="pchat" id="' +
          val +
          '">' +
          val +
          '</a><span><span id="typing_' +
          val +
  ...
  socket.on("getprivatemsg", function (sender, key, message) {
    ...
    if (sender == key && count == 0) {
      alert(
        "You are chatting with yourself, Server will repeat what ever you'll send! By this you can test yourself if no user online except you!"
      );
      count++;
    }
  ...
  function private_send(key) {
    ...
    $("#to_" + key + "_text").append(
      "<div><b>Me: </b>" + messagetosend + "</div>"
    );
...
function progressBar(totalOnlineUser) {
  if (totalOnlineUser) {
    ...
    $(".modal-body").html(
      "<img src='/images/loading.GIF'><h3>  Loading please wait....</h3>"
    );
  }
}
